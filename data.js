...
const portfolioItems = [
  ...
  },
  {
    ...
    links: [],
    detailNarrative:
      "我平时就会用 Vibe Coding 做一些小而具体的浏览器插件：有的是为了陪伴自己工作，比如把家里养的猫做成会出现在浏览器里的小猫桌宠；有的是为了减少琐碎计算，比如桌面计算器、工作时间提醒这类即时反馈工具。它们不一定是大型系统，但都来自真实的工作和生活需求，能把“想要一个顺手小工具”的念头快速变成可用界面。在美团 Tabbit 浏览器发布前期，我把自己做的“下班倒计时 + 日薪到手统计”插件上传参赛，用它把剩余工作时间和实时收入反馈可视化，既提供一点工作中的情绪陪伴，也让日常节奏更直观，最终获得了首届妙招脚本大赛优秀奖。",
    excerpt: "功能 + 场景 + 效果：辅助数据统计、推文形式优化、运营流程自动化，提升日常运营效率。"
  },
  {
    id: "ai-dashboard",
    title: "No Code自动化数据看板",
    category: "ai",
    description:
      "在明镜专项中使用No Code（AI工具）搭建自动化数据看板，为各大区执行与定期数据播报提供运营支撑。",
  ...
  },
  {
    ...
    links: [],
    detailNarrative:
      "每月廉正案件通报都需要先整理案件数据，再把不同类型的案例转化成适合推文发布的结构和排版代码，这类工作规则明确但非常耗时。我设计 Violation-Reporting-Skill，是为了让 AI 接管其中最重复的部分：读取月度廉正案件数据，按照通报逻辑完成归类、结构整理，并直接生成可用于推文排版的 HTML 代码。这样团队成员可以把精力放在内容审核、风险表达和传播判断上，而不是反复手动排版，最终为团队节省约4人日/月。",
  ...
  },
  {
    ...
    links: [],
    detailNarrative:
      "这个工具来自一个很具体的家庭需求：长辈希望持续背诗，但纸质记录和聊天收藏都很难长期追踪进度。我把这个需求拆成诗词录入、搜索管理、背诵状态、复习入口和学习笔记几个动作，做成一个适合手机使用的诗词背诵管理系统。它既能记录已经背过的诗，也能进入单篇详情查看正文、释义、作者信息并留下笔记，还可以通过复习页重新巩固记忆；对使用者来说，它把零散的背诵任务变成了一个可查看、可复习、可持续积累的学习流程。",
  ...
  },
  {
    ...
    category: "ai",
    description:
      "为家庭阅读场景定制的 AI 朗读工具，用于管理长文本阅读材料，并将文章内容转化为可随时播放的听读任务，适合通勤、散步或碎片时间继续阅读。",
    ...
    links: [],
    detailNarrative:
      "AI倍速朗读工具也是从家庭阅读场景出发做的：有些长文本材料值得读，但并不总有完整时间坐下来阅读。我希望把这些材料变成可以随时调用的听读任务，因此设计了阅读材料管理、历史统计、新建阅读、字数统计和一键朗读功能。使用者可以把文章放进工具里，之后在通勤、散步或做家务时继续收听，既保留了阅读材料的管理秩序，也把原本被搁置的长文本转化成更容易被完成的音频化学习内容。",
